...
## What is WireGuard?
Before explaining what WireGuard is I think it is important to understand what a VPN generally is.
...
WireGuard is a relatively new open source VPN solution that uses state-of-the-art cryptography. It aims to be faster and more simpler to learn than other alternatives. On the WireGuard website it states the following 5 principles[^2]: 
 - Simple & Easy-to-use
...
### Is WireGuard Safe?
According to [All About Cookies](https://allaboutcookies.org/) WireGuard is considered to be "one of the safest, most secure VPN protocol options available today."[^3] It also has the added benefit that it is used by popular VPN software such as [NordVPN](https://nordvpn.com/nord-site/), [SurfShark](https://surfshark.com/), [CyberGhost](https://www.cyberghostvpn.com/en_US/), and [MullVAD VPN](https://mullvad.net/en).

WireGuard does have a security flaw out of the box though. When using other services such as OpenVPN you are assigned a different IP address each time you connect, though with WireGuard your IP address remains static and unchanging. "This is faster, but it means the VPN server must keep logs of your real IP address and connection timestamps."[^4] It should be mentioned though that the VPN providers using WireGuard circumvented this issue by modifying WireGuard with their own systems to get around this flaw. In this guide we won't be covering that since that is likely way out of my expertise.

[^3]: https://en.wikipedia.org/wiki/Virtual_private_network
[^4]: https://www.tomsguide.com/how-to/is-the-new-wireguard-protocol-secure
